...
export default function TheoryAssessment({ skillId, skillName, questions, userId }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (error) {
        console.error("Supabase Error saving theory results:", error);
        toast.error("Failed to save results. Check your connection.");
      } else {
      ...
      console.error("Exception saving theory results:", e);
          ...
          <Card className="shadow-elevated border-border/50 animate-fade-in">
            ...
            <CardContent className="space-y-6">
              ...
              <div className="flex flex-col gap-3">
                {results.passed ? (
                  <Button className="w-full gradient-accent border-0" onClick={() => navigate(`/assessment/${skillId}/3`)}>
                    Proceed to Level 3 (Practical)
