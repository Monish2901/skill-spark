...
export default function MCQAssessment({ skillId, skillName, questions, userId }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    const negativeMarks = Math.floor(incorrect / 2);
    ...
    try {
      await supabase.from("assessment_attempts").insert({
        user_id: userId, skill_id: skillId, level: 1,
